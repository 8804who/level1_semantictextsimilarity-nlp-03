...
if __name__ == '__main__':
    ...
    parser.add_argument('--predict_path', default='./data/checked_test.csv')
    ...
    for path in paths:
        ...
        predictions.append(list(float(i) for i in torch.cat(prediction)))

    ensemble_prediction = []

    for v1, v2 in zip(*predictions):
        n1, n2 = sigmoid(v1), sigmoid(v2)
        avg = (n1+n2)/len(paths)
        ...
        if val<0:
            val=0
        ensemble_prediction.append(val)

    # output 형식을 불러와서 예측된 결과로 바꿔주고, output.csv로 출력합니다.
    output = pd.read_csv('./data/sample_submission.csv')
    output['target'] = ensemble_prediction
