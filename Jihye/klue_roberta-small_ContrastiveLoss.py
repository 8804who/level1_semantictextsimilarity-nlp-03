...
class ContrastiveLoss(torch.nn.Module):
    ...
    def forward(self, logits, labels):
        ...
        margin_square = torch.square(torch.max(self.margin - logits, torch.zeros_like(logits)))
        loss = torch.mean(labels * square_logits + (1 - labels) * margin_square)
...
if __name__ == '__main__':
    ...
    wandb_logger = WandbLogger(name="klue_roberta-small_ContrastiveLoss", project="klue-sts_03")
    cfg = {
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        monitor="val_pearson",
        mode="max",
        dirpath="./model/klue_roberta-small_ContrastiveLoss/", 
    ...
    # 학습이 완료된 모델을 저장합니다.
    torch.save(model, './model/klue_roberta-small_ContrastiveLoss/model.pt')
